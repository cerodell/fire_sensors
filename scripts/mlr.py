...
ax.plot(ramp_df.index,ramp_df['pm25'], lw = 2, label = f'RAMP          Mean: {int(ramp_df["pm25"].mean())}' + r" ($\frac{\mu g}{m^3}$)")
...
def make_mlr(i):
  ...
  ax.set_title(f"UBC-PM-{ubc_pms[i]}  MLR " +r"$R^{2}$" + f"= {round(R2_MLR,4)} \n y = {round(coeff['a0'],4)} + {round(coeff['a1'],4)}x")
  return 
...
fig.tight_layout()
fig.savefig(
    str(img_dir) + f"/{file_date}-mlr.png", dpi=250, bbox_inches = 'tight'
)
